...
with open('commands_input.txt', 'w') as f:
    for i in range(numCores[0]):
        for j in range(numCores[1]):
            f.write(" " + str(Na) + " " + str(aValCut[i]) + " " + str(aValCut[i+1]) + " " + str(Nb) + " " + str(bValCut[j]) + " " + str(bValCut[j+1])+" /p/QSOpt/NAEopt/optZ/runZn4k2resExtplus/part"+ "%03d" % (ind+1) +"/ \n")
            ind = ind+1
